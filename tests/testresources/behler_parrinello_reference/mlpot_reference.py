...
for t in atom_types:
    ...
    feed_dict[model.atomic_contributions[t].derivatives_input] = dGs_by_type[t]
...
with h5py.File("saved_model.h5", "w") as f:
    ...
    f.attrs["layer_names"] = np.array(
        [b"atomic_neural_network", b"atomic_neural_network_1"]
    )
    f.create_dataset(
        "atomic_neural_network/dense/kernel:0", data=variables[0], dtype="f4"
    )
    f.create_dataset(
        "atomic_neural_network/dense/bias:0", data=variables[1], dtype="f4"
    )
    f.create_dataset(
        "atomic_neural_network/dense_1/kernel:0", data=variables[2], dtype="f4"
    )
    f.create_dataset(
        "atomic_neural_network/dense_1/bias:0", data=variables[3], dtype="f4"
    )
    f["atomic_neural_network"].attrs["weight_names"] = np.array(
        [b"dense/kernel:0", b"dense/bias:0", b"dense_1/kernel:0", b"dense_1/bias:0"]
    )

    f.create_dataset(
        "atomic_neural_network_1/dense_2/kernel:0", data=variables[4], dtype="f4"
    )
    f.create_dataset(
        "atomic_neural_network_1/dense_2/bias:0", data=variables[5], dtype="f4"
    )
    f.create_dataset(
        "atomic_neural_network_1/dense_3/kernel:0", data=variables[6], dtype="f4"
    )
    f.create_dataset(
        "atomic_neural_network_1/dense_3/bias:0", data=variables[7], dtype="f4"
    )
    f["atomic_neural_network_1"].attrs["weight_names"] = np.array(
        [b"dense_2/kernel:0", b"dense_2/bias:0", b"dense_3/kernel:0", b"dense_3/bias:0"]
    )
