...
def test_saving_and_loading(resource_path_root, tmpdir):
    ...
    type_dict = {"Ni": 0, "Pt": 1}
    types = tf.ragged.constant([[[type_dict[t]] for t in data["types"]]], ragged_rank=1)
    Gs = tf.ragged.constant([data["Gs"]], ragged_rank=2)
    dGs = tf.ragged.constant([data["dGs"]], ragged_rank=3)

    model = BehlerParrinello(
        ["Ni", "Pt"], num_Gs={"Ni": 50, "Pt": 50}, build_forces=True
    )
    ...
    model2.load_weights(tmpdir / "tmp_model.h5")
    prediction2 = model2({"types": types, "Gs": Gs, "dGs": dGs})
    energy2, forces2 = (prediction2["energy_per_atom"], prediction2["forces"])

    np.testing.assert_allclose(energy2.numpy(), energy.numpy())
    np.testing.assert_allclose(forces2.numpy(), forces.numpy())


def test_behlerparinello_versus_mlpot(resource_path_root, tmpdir):
    print(resource_path_root)
    tf.keras.backend.clear_session()
    with open(
        resource_path_root / "behler_parrinello_reference" / "mlpot_results.json", "r"
    ) as fin:
        data = json.load(fin)
