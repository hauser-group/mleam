...
class BehlerParrinello(tf.keras.Model):
    ...
    def __init__(self, atom_types, num_Gs, layers=None, build_forces=False):
        super().__init__()
        ...
        self.atomic_neural_nets = {
            t: AtomicNeuralNetwork(layers[t]) for t in atom_types}
        ...
        if self.build_forces:
            inputs['dGs'] = tf.keras.Input(shape=(None, None, None, 3),
                                           ragged=True)
    ...
    def main_body_with_forces(self, types, Gs, dGs):
        ...
        dE_dG = tf.expand_dims(tf.RaggedTensor.from_nested_row_splits(
            tape.gradient(energy, Gs.flat_values),
            Gs.nested_row_splits, name='dE_dG'), -1)
    ...
    def body_partition_stitch(self, types, Gs):
        ...
        type_indices = tf.dynamic_partition(
            ...
            types.flat_values, 2, name='type_indices')
        ...
        atomic_energies = tf.RaggedTensor.from_row_splits(
            ...
            types.row_splits, name='atomic_energies')
