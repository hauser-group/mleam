...
def get_pair_type(types, n):
    ...
    return tf.ragged.boolean_mask(
        n*min_ij - (min_ij*(min_ij-1))//2 + tf.abs(tf.subtract(types[:, tf.newaxis], types[tf.newaxis, :])),
        mask)
...
def distances_with_gradient(xyz):
    ...
    # Expanding the mask dimension is necessary to ensure that the gradient has ragged_rank = 2.
    # mask2 = tf.broadcast_to(tf.expand_dims(mask, -1), (n, n, n))
    ...
    indices = tf.stack([tf.broadcast_to(tf.expand_dims(tf.range(n), 1), (n, n)),
    ...
    # Slicing into the masked tensor would be significanlty more difficult but could be achieved 
    # similar to this n=4 example
...
def distances_and_pair_types(xyz, types, n_types, cutoff=10.0):
    ...
    min_ij = tf.math.minimum(types[:, tf.newaxis], types[tf.newaxis, :])
    pair_types = (n_types*min_ij - (min_ij*(min_ij-1))//2 
