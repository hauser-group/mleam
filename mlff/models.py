...
class EAMPotential(tf.keras.Model):
    ...
    def __init__(self, atom_types, build_forces=False):
        ...
        self.pair_potentials, self.pair_rho, self.embedding_functions = self.build_functions()
    ...
    def body_partition_stitch(self, types, distances, pair_types):
        ...
        rho = [self.pair_rho[t](part) for t, part in zip(self.atom_pair_types, partitioned_r)]
        phi = [self.pair_potentials[t](part) for t, part in zip(self.atom_pair_types, partitioned_r)]
        ...
        # Embedding energy
        partitioned_sum_rho = tf.dynamic_partition(sum_rho * 1, types, len(self.atom_types), name='partitioned_sum_rho')
        type_indices = tf.dynamic_partition(tf.expand_dims(tf.range(tf.size(sum_rho)), -1) * 1, types.flat_values , len(self.atom_types), name='type_indices')
        embedding_energies = [self.embedding_functions[t](rho_t) for t, rho_t in zip(self.atom_types, partitioned_sum_rho)]
        atomic_energies = (sum_phi.flat_values
                           + tf.expand_dims(tf.dynamic_stitch(type_indices, embedding_energies * 1), -1))
        # Reshape to ragged
        atomic_energies = tf.RaggedTensor.from_row_splits(atomic_energies, types.row_splits)
        # Sum over atoms i
    ...
    def body_gather_scatter(self, types, distances, pair_types):
        ...
        for ij, type_ij in enumerate(self.atom_pair_types):
            # Flat values necessary until where properly supports ragged tensors
            ...
            phi = tf.tensor_scatter_nd_update(
                phi, tf.expand_dims(indices, -1), self.pair_potentials[type_ij](masked_distances))
            rho = tf.tensor_scatter_nd_update(
                rho, tf.expand_dims(indices, -1), self.pair_rho[type_ij](masked_distances))
...
class DeepEAMPotential(EAMPotential):
    ...
    def call(self, inputs):
        ...
        with tf.GradientTape() as tape:
            ...
            #distances, pair_types = calc_distances_and_pair_types(positions, types, len(self.atom_types))
        ...
        energy_per_atom = energy/tf.expand_dims(tf.cast(types.row_lengths(), tf.float32, name='number_of_atoms'), -1)
...
class ShallowEAMPotential(EAMPotential):
    ...
    def __init__(self, atom_types, **kwargs):
        ...
        pair_types = tf.keras.Input(shape=(None, None, 1), ragged=True, dtype=tf.int32)
    ...
    def call(self, inputs):
        ...
        energy_per_atom = energy/tf.expand_dims(tf.cast(types.row_lengths(), tf.float32, name='number_of_atoms'), axis=-1)
        ...
        if self.build_forces:
            ...
            gradients = tf.reduce_sum(dr_dx * tf.expand_dims(dE_dr, -1), axis=(-3, -4))
            return energy_per_atom, gradients
        return energy_per_atom
...
class SMATB(ShallowEAMPotential):
    ...
    def build_functions(self):
        ...
        for (t1, t2) in combinations_with_replacement(self.atom_types, 2):
            ...
            rho = RhoExp(type_i, xi=self.initial_params.get(('xi', type_i), 1.6),
                         q=self.initial_params.get(('q', type_i), 3.5))
            pair_potentials[type_i] = PairInteraction(
                normalized_input, pair_potential, cutoff_function, name='%s-phi'%type_i)
